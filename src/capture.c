...
struct capture{
    ...
    size_t max_frame_size; // 各块里最大的那个：驱动一帧最多写这么多
...
capture *cap_create(const capture_config *cfg)
{
    ...
    /* 不加 O_NONBLOCK：要的就是阻塞 DQBUF，靠 STREAMOFF 让它返回错误来收线程 */
    ...
    /* 设置格式，S_FMT 成功后 fmt 会被驱动改成实际支持的格式，
    ...
    /* 驱动可能把我们请求的格式换成它支持的别的，不认识的格式必须拦下来：
     * 否则 fourcc_to_fmt 会把它当成 YUYV，画面是花的却没有任何报错 */
    ...
    /* 逐块 QUERYBUF（问长度和偏移）→ mmap（映射进用户空间）→ QBUF（还给驱动） */
    for (unsigned i = 0; i < req.count; i++) {
        ...
        if (buf.length > cap->max_frame_size) cap->max_frame_size = buf.length;
        mapped++;   // 多成功映射了一块，失败回滚时只还这些

        if (xioctl(cap->fd, VIDIOC_QBUF, &buf, "VIDIOC_QBUF") < 0) goto fail;
    }
    printf("成功映射 %d 个缓冲区，每块 %zu 字节\n", mapped, cap->max_frame_size);
...
static void *capture_thread(void *arg)
{
    ...
    for (;;) {
        ...
        /* 返回值没看：帧比队列槽还大时会被丢掉，那是配置错误，
         * cap_start 已经校验过槽容量，正常不会走到那条路 */
        fq_push(cap->fq, cap->buffers[buf.index], buf.bytesused);

        ioctl(cap->fd, VIDIOC_QBUF, &buf);   // 归还缓冲；退出阶段失败也无所谓
    }
    return NULL;
}
...
int cap_start(capture *cap, frame_queue *fq)
{
    ...
    /* 队列的槽必须装得下驱动可能写出的最大帧，MJPEG 每帧长度不定，
     * 所以是 >= 而不是 ==（YUYV 下两者相等） */
    if (fq_max_size(fq) < cap->max_frame_size) return -1;
...
{
    return cap ? cap->max_frame_size : 0;
}
