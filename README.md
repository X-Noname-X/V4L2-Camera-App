...
`cmake ..` 只生成构建系统、不编译，`make` 才真正调 gcc。CMake 3.13 起这两步也能写成 `cmake -S . -B build` + `cmake --build build`，省掉 `cd`，做的事一样。
...
拿它和上一版对跑。上一版是单线程的：`poll+DQBUF → 解码 → 上屏 → QBUF` 全在一个循环里，4 块 V4L2 缓冲，**整个解码上屏期间缓冲都被它攥着不还**。同机、同相机、同样设置。

### 1. 相机是瓶颈时：没有区别

| 场景 | 上一版 | 本版 |
|---|---|---|
| 640x480 MJPEG | 30.4 fps | 29.7 fps |
| 1280x720 MJPEG | 30.2 fps | 29.9 fps |

两边都顶在相机的 30fps 上。**多一个线程不会让帧率变高**——相机的产出速度没变，消费端也没变。

### 2. 消费端恒定慢时：也没有区别

每帧故意多花 100ms（720p，150 帧）：

| | 吞吐 | 丢帧 |
|---|---|---|
| 上一版 | 9.0 fps | 342 帧（69.5%） |
| 本版 | 9.0 fps | **340 帧（69.4%）** |

消费端就是瓶颈，多一个线程变不出更多算力；而且此时两边的缓冲都常年填满，丢的是同一批帧。

区别只有**报不报**：上一版那 342 帧丢得无声无息，本版把 69.4% 打在了屏幕上。

### 3. 消费端突发卡顿时：差距在这里

每 30 帧卡 200ms（640x480，300 帧）：

| 轮次 | 上一版 | 本版 |
|---|---|---|
| 1 | 26.7 fps，丢 41 帧（12.0%） | 29.9 fps，**丢 0** |
| 2 | 26.5 fps，丢 44 帧（12.8%） | 29.9 fps，**丢 0** |
| 3 | 26.6 fps，丢 45 帧（13.0%） | 29.9 fps，**丢 0** |
| 4 | 26.6 fps，丢 44 帧（12.8%） | 29.9 fps，**丢 0** |

4/4 轮稳定复现。上一版的丢帧数是靠 `buf.sequence`（驱动给帧的连续编号）首尾相减算出来的——**这是唯一能看见它的办法**，那个版本自己什么都不会报。

为什么差这么多，把上一版的每帧耗时拆开就清楚了：

```
基线:  poll+DQBUF 8.41s / 解码+上屏 1.46s            → 每帧等 28.0ms + 干活 4.9ms
突发:  poll+DQBUF 7.96s / 解码+上屏 1.33s / 卡 2.00s  → 每帧等 26.5ms + 干活 4.4ms
```

**它每帧真正干活只有 4.4ms**——相机帧间隔 33.3ms，其余时间基本都在 `poll` 里等下一帧，瓶颈根本不在算力。所以它丢帧不是处理不过来，而是：卡顿那 200ms 里 V4L2 缓冲被它自己攥着，驱动无处可写，**这期间的帧直接永久丢失**。

本版的卡顿发生在帧**已经 memcpy 进队列之后**，采集线程照常 DQBUF/QBUF，8 个槽（8 × 33.3ms = 267ms）把 200ms 的卡顿整个吸收掉。

> **多一个线程不是让它算得更快，是让它在卡顿期间还能收帧。**

### 这个丢帧计数只覆盖一半

`fq_dropped` 统计的是**队列**丢帧。如果驱动自己丢帧（`buf.sequence` 跳号），本版和上一版一样看不出来——`src/capture.c` 读了 `buf.index` 和 `buf.bytesused`，**没读 `sequence`**。

上面第 3 组里驱动确实没丢，但那是**结构保证的**（采集线程总是及时归还缓冲），不是计数保证的。要坐实得把 `sequence` 也记下来，把「驱动丢帧」和「队列丢帧」分开报——现在两者混在一个数里。

### 一个反过来的代价

队列 8 槽 + `DROP_OLDEST`，稳态过载时队列常满，弹出的永远是第 8 新的帧 ≈ **267ms 前**；上一版的等效缓冲是 3 块（4 块减去手里那块）≈ **100ms**。也就是**过载时本版的画面更旧约 167ms**（推导，未实测）。

此时丢帧率本来就 >69%，画面新旧意义不大，但这是个真实的取舍：**队列容量调小就是拿抗突发换低延迟**。
